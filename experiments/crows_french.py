...
import transformers
import sys
sys.path.append('/data/leuven/344/vsc34470/bias-bench/')
os.chdir('/data/leuven/344/vsc34470/bias-bench/')
print(os.listdir())
from bias_bench.benchmark.crows import CrowSPairsRunner_french
...
if __name__ == "__main__":
    ...
    runner = CrowSPairsRunner_french(
        model=model,
        tokenizer=tokenizer,
        input_file=f"{args.persistent_dir}/data/crows/crows_pairs_FR.csv",
        bias_type=args.bias_type,
    ...
    with open(f"{args.persistent_dir}/results/crows/{experiment_id}_fr.json", "w") as f:
        json.dump(results, f)
