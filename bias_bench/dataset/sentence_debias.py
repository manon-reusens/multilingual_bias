...
def load_sentence_debias_data(persistent_dir, bias_type,lang_debias):
    data = []
    for dataset_name in DATASET_NAMES:
        if dataset_name == "sst":
            ...
            dataset = _GenericDataset(persistent_dir, bias_type, dataset_name,lang_debias=lang_debias)
...
class _SentenceDebiasDataset:
    ...
    def __init__(self, persistent_dir, bias_type,lang_debias):
        ...
        if self._lang_debias=='en':
            ...
            with open(f"{self._persistent_dir}/data/bias_attribute_words_fr.json", "r") as f:
                self._attribute_words = json.load(f)[self._bias_type]
...
class _GenericDataset(_SentenceDebiasDataset):
    def __init__(self, persistent_dir, bias_type, name,lang_debias):
        super().__init__(persistent_dir, bias_type,lang_debias)
        #self._name = name
